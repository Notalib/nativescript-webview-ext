...
webViewBridgeJsCodePromise.then(
    (scriptCode) => (webViewBridgeJsCode = scriptCode),
);
...
export class WebViewExt extends WebViewExtBase {
    ...
    private _wkUserContentController: WKUserContentController;
    private _wkUserScriptInjectWebViewBrigde: WKUserScript;
    private _wkUserScriptViewPortCode: WKUserScript;
    private _wkNamedUserScripts: Array<{
        name: string;
    ...
    private initIOS11Plus() {
        ...
        this._wkNavigationDelegate = WKNavigationDelegateImpl.initWithOwner(
            new WeakRef(this),
        );

        const jsBridgeScript = `
            ${webViewBridgeJsCode.trim()};
        `;

        this._wkUserScriptInjectWebViewBrigde = this.createWkUserScript(
            jsBridgeScript,
        );
        ...
        const messageHandler = WKScriptMessageHandlerImpl.initWithOwner(
            new WeakRef(this),
        );
        ...
        this._wkWebViewConfiguration.setURLSchemeHandlerForURLScheme(
            this._wkCustomUrlSchemeHandler,
    ...
    private initIOS9and10() {
        ...
        this._uiWebViewDelegate = UIWebViewDelegateImpl.initWithOwner(
            new WeakRef(this),
        );
    ...
    public executeJavaScript<T>(
        ...
        return new Promise((resolve, reject) => {
            if (this._wkWebView) {
                this._wkWebView.evaluateJavaScriptCompletionHandler(
                    scriptCode,
                    (result, error) => {
                        if (error) {
                            reject(error);
                ...
                try {
                    const result = this._uiWebView.stringByEvaluatingJavaScriptFromString(
                        scriptCode,
                    );
                    resolve(result);
                } catch (error) {
    ...
    public _loadUrl(src: string) {
        ...
        if (this._wkWebView) {
            if (src.startsWith("file:///")) {
                ...
                this.writeTrace(
                    `WebViewExt<ios>._loadUrl("${src}") -> this._wkWebView.loadFileURLAllowingReadAccessToURL("${nsURL}", "${nsReadAccessUrl}"`,
                );
                this._wkWebView.loadFileURLAllowingReadAccessToURL(
                    nsURL,
                    nsReadAccessUrl,
                );
            } else {
                ...
                this.writeTrace(
                    `WebViewExt<ios>._loadUrl("${src}") -> this._wkWebView.loadRequest("${nsRequestWithUrl}"`,
                );
            ...
            const nsRequestWithUrl = NSURLRequest.requestWithURL(nsURL);
            this.writeTrace(
                `WebViewExt<ios>._loadUrl("${src}") -> this._uiWebView.loadRequest("${nsRequestWithUrl}"`,
            );
    ...
    public _loadData(content: string) {
        const nsURL = NSURL.alloc().initWithString(
            `file:///${fs.knownFolders.currentApp().path}/`,
        );
        if (this._wkWebView) {
            this.writeTrace(
                `WebViewExt<ios>._loadUrl(content) -> this._wkWebView.loadHTMLStringBaseURL("${nsURL}")`,
            );
            this._wkWebView.loadHTMLStringBaseURL(content, nsURL);
        } else if (this._uiWebView) {
            this.writeTrace(
                `WebViewExt<ios>._loadUrl(content) -> this._uiWebView.loadHTMLStringBaseURL("${nsURL}")`,
            );
    ...
    public registerLocalResource(resourceName: string, path: string) {
        ...
        if (!filepath) {
            this.writeTrace(
                `WebViewExt<ios>.registerLocalResource("${resourceName}", "${path}") -> file doesn't exist`,
        ...
        this.writeTrace(
            `WebViewExt<ios>.registerLocalResource("${resourceName}", "${path}") -> file: "${filepath}"`,
        );

        if (this._wkWebView) {
            this._wkCustomUrlSchemeHandler.registerLocalResourceForKeyFilepath(
                resourceName,
                filepath,
            );
        } else if (this._uiWebView) {
            CustomNSURLProtocol.registerLocalResourceForKeyFilepath(
                resourceName,
                filepath,
            );
        }
    ...
    public unregisterLocalResource(resourceName: string) {
        this.writeTrace(
            `WebViewExt<ios>.unregisterLocalResource("${resourceName}")`,
        );

        resourceName = this.fixLocalResourceName(resourceName);

        if (this._wkWebView) {
            this._wkCustomUrlSchemeHandler.unregisterLocalResourceForKey(
                resourceName,
            );
        } else if (this._uiWebView) {
    ...
    public getRegistretLocalResource(resourceName: string) {
        ...
        let result: string;
        if (this._wkWebView) {
            result = this._wkCustomUrlSchemeHandler.getRegisteredLocalResourceForKey(
                resourceName,
            );
        } else if (this._uiWebView) {
            result = CustomNSURLProtocol.getRegisteredLocalResourceForKey(
                resourceName,
            );
        } else {
        ...
        this.writeTrace(
            `WebViewExt<android>.getRegistretLocalResource("${resourceName}") -> "${result}"`,
        );
        return result;
    }
    ...
    public onUIWebViewEvent(url: string) {
        if (!this.isUIWebView) {
            this.writeTrace(
                `WebViewExt.onUIWebViewEvent("${url}") - only works for UIWebView`,
                traceMessageType.error,
            );
            return;
        }
        ...
        if (!url.startsWith("js2ios")) {
            this.writeTrace(
                `WebViewExt.onUIWebViewEvent("${url}") - only supports js2ios-scheme`,
                traceMessageType.error,
            );
            return;
        }
        ...
        try {
            ...
            this.executeJavaScript<any>(
                `window.nsWebViewBridge.getUIWebViewResponse(${JSON.stringify(
                    resId,
                )})`,
            )
                .then((data) => {
                ...
                .catch((err) => {
                    this.writeTrace(
                        `WebViewExt.onUIWebViewEvent("${url}") - getUIWebViewResponse - ${err}`,
                        traceMessageType.error,
                    );
                });
        } catch (err) {
            this.writeTrace(
                `WebViewExt.onUIWebViewEvent("${url})" - "${err}"`,
                traceMessageType.error,
            );
        }
    }
    ...
    public autoLoadStyleSheetFile(
        ...
        if (this._wkWebView) {
            ...
            this.addNamedUserScript(
                `auto-load-css-${resourceName}`,
                scriptCode,
            );
        } else {
            return super.autoLoadStyleSheetFile(
                resourceName,
                filepath,
                insertBefore,
            );
    ...
    private addNamedUserScript(name: string, scriptCode: string) {
        if (!this._wkWebView) {
            return;
        }
    ...
    private createWkUserScript(scriptCode: string) {
    ...
    private removeNamedUserScript(name: string) {
        if (!this._wkWebView) {
            return;
        }

        const idx = this._wkNamedUserScripts.findIndex(
            (val) => val.name === name,
        );
        if (idx === -1) {
            return;
    ...
    private loadWKUserScripts(autoInjectJSBridge = this.autoInjectJSBridge) {
        if (!this._wkWebView) {
            return;
        }
        ...
        if (!autoInjectJSBridge) {
            this._wkUserContentController.addUserScript(
                this._wkUserScriptViewPortCode,
            );
            return;
        }

        this._wkUserContentController.addUserScript(
            this._wkUserScriptInjectWebViewBrigde,
        );
        this._wkUserContentController.addUserScript(
            this._wkUserScriptViewPortCode,
        );
