...
export const ASYNC = 0.2;
export const MEMORY_ASYNC = 2;
...
const props = (properties as any)._getProperties();
const styleProps = (properties as any)._getStyleProperties();
let setters: Map<string, any>;
let cssSetters: Map<string, any>;
let defaultNativeGetters: Map<string, (view) => any>;

export function nativeView_recycling_test(
    createNew: () => View,
    createLayout?: () => LayoutBase,
    nativeGetters?: Map<string, (view) => any>,
    customSetters?: Map<string, any>,
) {
    return;
...
function compareUsingReflection(recycledNativeView: View, newNativeView: View): void {
    ...
    for (let i = 0, length = methods.length; i < length; i++) {
        ...
        if ((<any>java).lang.Comparable.class.isAssignableFrom(returnType)) {
            ...
            java.lang.String.class === returnType ||
...
function setupSetters(): void {
    ...
    setters.set(
        "itemTemplates",
        '<template key="green"><Label text="{{ $value }}" style.backgroundColor="green" /></template><template key="red"><Label text="{{ $value }}" style.backgroundColor="red" /></template>',
    );
